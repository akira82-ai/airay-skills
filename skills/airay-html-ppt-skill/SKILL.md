...
启动后首次提示用户按下面格式提供信息（示例可直接改）：

```text
0）查看demo（如：先看示例效果再决定是否开始制作）

1）主题（如：AI 时代的产品叙事重构：从功能堆叠到认知设计的长期主义）

2）受众（如：80人、产品经理、设计师、研发负责人、AI初学者、业务管理层、跨部门协作、结果导向）

3）时长（如：40 分钟）

4）目的（如：行业分享 / 产品发布 / demo day / 私享会）

5）页数要求（如：30 页）
```

说明：该提示仅在 skill 启动时首次展示一次，后续按 Step 0~Step 6 正常推进。
