...
def get_page_size_inches(doc):
    section = doc.sections[0]
    return section.page_width / 914400, section.page_height / 914400
...
def crop_image_to_ratio(image_path, target_ratio):
    with PILImage.open(image_path) as img:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="airay-cover-", suffix=".jpg", delete=False
        )
        tmp.close()
        cropped.save(tmp.name, format="JPEG", quality=95)
        return tmp.name
...
def load_builtin_author_page(preset_name):
    ...
    if not preset_path or not preset_path.exists():
        print(f"❌ 找不到作者页模板：{preset_name}", file=sys.stderr)
        sys.exit(1)
...
CHAPTER_PREFIX_RE = re.compile(
    r"^(第\s*[一二三四五六七八九十百千万0-9]+\s*章|第\s*\d+\s*章|附录|后记|前言|序)\s*"
)
...
# 封面 / 目录
# ============================================================
def add_cover_image_panel(doc, image_path):
    if not image_path or not Path(image_path).exists():
        return False

    page_width_inches, page_height_inches = get_page_size_inches(doc)
    page_ratio = page_width_inches / page_height_inches if page_height_inches else 1
    cropped_path = crop_image_to_ratio(image_path, page_ratio)
    ...
    row = table.rows[0]
    row.height = Inches(page_height_inches)
    ...
    r = p.add_run()
    r.add_picture(str(cropped_path), width=Inches(page_width_inches),
                  height=Inches(page_height_inches))
    return True
...
def parse_author_page_blocks(text):
    ...
    for raw in text.split("\n\n"):
        ...
        if not block:
            continue
        if block.startswith("## "):
            blocks.append({"kind": "heading", "text": block[3:].strip()})
        elif re.match(r"^https?://\S+$", block):
            blocks.append({"kind": "link", "text": block})
        else:
            blocks.append({"kind": "paragraph", "text": block.replace("\n", " ")})
    return blocks


def add_author_page(doc, text):
    blocks = parse_author_page_blocks(text)
...
def configure_author_page(section):
    section.page_width = Cm(17.6)
    section.page_height = Cm(24.0)
